...
def emergency_call_tool() -> str:
    ...
    Returns a short status message describing the call result (call SID, sms fallback, or failure).
    """
    result = call_emergency()
    # Ensure tool returns a short status string so the agent and logs show what happened
    try:
        return str(result)
    except Exception:
        return "call_tool_executed_but_no_status"
